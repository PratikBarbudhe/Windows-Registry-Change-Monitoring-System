...
def read_registry_key(path: str) -> Dict[str, Any]:
    """Read one key and return a lightweight dictionary of value name -> data."""
    key_data = list_registry_values(path)
    if not key_data.get("exists"):
        return {
            "path": path,
            "exists": False,
            "values": {},
